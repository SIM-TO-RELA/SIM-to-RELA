...
ROBOT_PATH = "xml/universal_robots_ur10e/ur10e_hande.xml"
...
kp=np.array([2000, 2000, 2000, 500, 500, 500, 100 ,100])
kv=np.array([200 , 200 , 200, 50, 50 ,50 ,10 ,10])
force_min=np.array([-150 , -150 , -150 , -28 , -28 , -28 ,-50 ,-50])
force_max=np.array([150 , 150 , 150 , 28 , 28 , 28 ,50 ,50])
...
for i in range(max(len(path), 1000)):
    try:
        franka.control_dofs_position(path[i] + 50)
        last_pt = path[i]
    except:
