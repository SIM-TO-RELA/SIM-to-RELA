# THIS FILE:
# 1) MOVES ROBOT TO INITIAL POSITION (-0.2, 0.02, 1.0)
# 2) WAITS FOR CUBE DETECTION FROM REALSENSE + YOLO
# 3) CONVERTS CAMERA COORDINATE TO GENESIS COORDINATE
# 4) MOVES ROBOT ONCE TO (gx, gy, 0.3)
...
# def cam_to_genesis(x_cam, y_cam):
#     x_gen = 0.9977 * x_cam - 0.1019 * y_cam - 0.2661
#     y_gen = -0.0169 * x_cam - 1.0021 * y_cam - 0.0632
#     return x_gen, y_gen
...
def main():
    ...
    # Wait for cube detection while robot stays there
    ...
    # Move once to cube location with constant z = 0.3
    cube_xyz = (gx, gy, 0.3)
    ok = move_robot_once(scene, entities, node, cube_xyz, move_time=5.0)
    if not ok:
        print("Could not move to cube target.")
